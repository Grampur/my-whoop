...
// MARK: - TodayWorkoutModal
// Lighter sheet shown when tapping a workout row on the Today tab.
// Same data as WorkoutDetailView but no delete action — just stats, zones, context, and tag.

struct TodayWorkoutModal: View {
    let workout: Workout
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear { selectedKind = WorkoutType(kind: workout.kind) }
    ...
    private var contextSection: some View {
        let hasHrmax = workout.hrmax != nil
        let hasHrr   = workout.avgHrrPct != nil
        guard hasHrmax || hasHrr else { return AnyView(EmptyView()) }
    ...
    private var formattedDate: String {
        ...
        let fmt = DateFormatter(); fmt.dateStyle = .full; fmt.timeStyle = .none
    ...
    private var formattedDuration: String {
        let h = workout.durationS / 3600
        let m = (workout.durationS % 3600) / 60
    ...
    private func formatZoneMins(_ mins: Double) -> String {
        mins < 1.0 ? "<1m" : "\(Int(mins.rounded()))m"
    }
