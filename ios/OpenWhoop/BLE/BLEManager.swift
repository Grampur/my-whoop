...
public final class BLEManager: NSObject, ObservableObject {
    ...
    /// WHOOP app uses for alarms (verified: patternId=2 from the APK mh0/b.java), plus RUN_ALARM
    /// (cmd 68) as a belt-and-suspenders. patternId=2 gives the characteristic graduated alarm buzz.
    ///
    /// Alternative waveform form (12-byte, from APK):
...
extension BLEManager: CBPeripheralDelegate {
    ...
    /// SET_CLOCK(10) payload = WHOOP's exact 8-byte form (apk lh0/s0.java): [seconds u32 LE][subseconds
            ...
            for frame in reassembler.feed(bytes) { autoreleasepool {
                ...
                if frame.count > 6, frame[6] == WhoopCommand.getAlarmTime.rawValue {
                    if frame.count >= 12 {
                        ...
                        if epoch == 0 {
                            log("Alarm ACK: strap reports NO alarm armed — SET_ALARM_TIME was rejected")
                        } else {
                            let date = Date(timeIntervalSince1970: TimeInterval(epoch))
                            let fmt = DateFormatter()
                            fmt.dateStyle = .short; fmt.timeStyle = .short
                            log("Alarm ACK: strap confirmed \(fmt.string(from: date)) (epoch \(epoch))")
                        }
                    } else {
                        log("Alarm ACK: response too short — frame=\(frame.map { String(format: \"%02x\", $0) }.joined())")
                    }
                }
