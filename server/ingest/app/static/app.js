...
const CAT = {
  frame: '#5b6e74', cmd: '#8fb0ff', time: '#39d6ff', hr: '#ff5a6e', rr: '#ff9f43',
...
const BIO = {
  red: '#ff5a6e', ir: '#ff9f43', skin: '#ffd23f', resp: '#2ee6c6',
...
async function openNight(day) {
  ...
  try {
    ...
    for (let i = 1; i < rr.length; i++) {
      const d = Math.abs(rr[i].rr_ms - rr[i - 1].rr_ms);
...
function drawChart(cv, pts, color, opts = {}) {
  ...
  if (validPts.length === 0) {
    x.fillStyle = '#3f5450'; x.font = '11px "IBM Plex Mono"';
...
function drawMultiChart(cv, series, opts = {}) {
  ...
  cv._hover = {
    t0, t1, padL, W, series: series.map((s, i) => ({
...
function typeColor(t) {
  return ({
    REALTIME_DATA: CAT.hr, REALTIME_RAW_DATA: CAT.accel, HISTORICAL_DATA: CAT.meta,
    EVENT: CAT.event, COMMAND_RESPONSE: CAT.cmd, METADATA: CAT.meta, CONSOLE_LOGS: CAT.text
  })[t] || CAT.unknown;
}
function apiKey() {
