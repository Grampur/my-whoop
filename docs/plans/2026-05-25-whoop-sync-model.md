...
that consults the policy. Periodic ticks are floored at 15 min (matching WHOOP's
`PeriodicWorkRequest(15, MINUTES)` in `ls0/b.java`); connect/foreground/manual may sync sooner but are
...
**Reference (the model we're mirroring):** WHOOP Android app, reverse-engineered:
`apk/src/sources/ls0/b.java` (`new i0.a(DataSyncWorker.class, 15L, MINUTES, 5L, MINUTES)` = 15-min
periodic + 5-min flex; plus expedited `DATA_SYNC_PROCESS_NOW` one-time syncs on triggers) +
`com/whoop/dataSyncService/workmanager/DataSyncWorker.java` (`STRAP_SYNC_RATE_LIMITING` flag +
`DATA_SYNC_WORKER_LAST_WORK_TIME` watermark). See `docs/specs/2026-05-25-debugging-runbook.md` §
backfill-cadence.
...
/// Floors chosen to match WHOOP (`ls0/b.java`: PeriodicWorkRequest 15-min; expedited event syncs).
enum BackfillPolicy {
...
UserDefaults `backfillLastAt`. Note it mirrors WHOOP's `ls0/b.java` 15-min PeriodicWorkRequest +
