...
`<DEVICE_SERIAL>`, harvard fw `41.16.6.0`, boylston `17.2.2.0`), cross-checked against the decompiled
WHOOP Android app v5.452.0 (`apk/src/sources/so0/e.java` = authoritative command enum) and the
`whoomp` reference. Supersedes the scattered notes in `FINDINGS.md` / `re/PROJECT_MEMORY.md` for
...
Authoritative list from `so0/c.java$a` (the app's enum) — supersedes the 10-entry list in the schema:
...
From `so0/e.java`. **Direction = app→strap** for all. Risk: 🟢 safe/read-only · 🟡 reversible
...
| 81 | START_RAW_DATA | `<u32 ms LE>` | — | **timed** R10/R11 raw capture, `hours×3_600_000` ms; auto-expires (app `mh0/d.java`). Prior RE's 1-byte payload was wrong. | ✅ | 🟡 |
...
| **151** | **GET_BATTERY_PACK_INFO** | `00` | `0a03` | **UNSUPPORTED on this fw** (new in v5.452 app) | ✅ | 🟢 |
| **153** | **TOGGLE_PERSISTENT_R20** | `[01, 0/1]` | `0a03` | **UNSUPPORTED on this fw** — payload `[REVISION_1, on]` per app `lh0/f1.java` | ✅ | 🟡 |
| **154** | **TOGGLE_PERSISTENT_R21** | `[01, 0/1]` | `0a03` | **UNSUPPORTED on this fw** — `lh0/g1.java` | ✅ | 🟡 |

> The three bold rows (151/153/154) and packet types 37/38/53–56 are **new in the v5.452 app and
...
**App-side confirmation (decompiled v5.452):** the app sends **no** persistent-mode / DP-type /
...
`TOGGLE_OPTICAL_MODE`(108)=1 (`rearchitect/c.java` `g()`); disable = same with `0`; plus the
realtime-HR trio `TOGGLE_REALTIME_HR`(3)=1 → `TOGGLE_IMU_MODE`=1 → `ABORT_HISTORICAL_TRANSMITS`
(`n81/b.java:224`). `START_/STOP_RAW_DATA` are used only by the opt-in WhoopLabs "DWL" campaign.
...
2. **`FORCE_DP_TYPE`(53) / `SET_DP_TYPE`(52)** — untested DP-type override; the only protocol lever
   left, but **no builder/reference exists in v5.452**, so the payload can't be recovered from this
...
6. A late-2024 (~v4.x) WHOOP APK could be decompiled (`jadx` available) to double-check the
   contemporary GEN_4 path, but apkmirror/apkcombo gate old versions behind JS/browser-UA so it
   needs a browser-driven fetch. Low priority given v5.452 already shows GEN_4 collection is autonomous.
