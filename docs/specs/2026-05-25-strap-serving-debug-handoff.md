...
- **iPhone** (named "Galaxy S9", actually iPhone 16 Pro). xcodebuild/devicectl device id (UDID):
  **`00008140-000158E80281401C`**. (devicectl also shows a CoreDevice id `F8C5B80B-333E-5C37-9457-2B8441A024A2`; use the UDID for builds.)
- **Code-signing team: `X4226N37ZF`** (this is the cert's **OU**, the real Team ID). The cert CN paren
  `L2KUMUA3H7` is the *user* id — do NOT use it as the team. Cert: "Apple Development: Johnathan Middleton".
...
xcodebuild -project OpenWhoop.xcodeproj -scheme OpenWhoop \
  -destination 'platform=iOS,id=00008140-000158E80281401C' \
  -allowProvisioningUpdates DEVELOPMENT_TEAM=X4226N37ZF build 2>&1 | tail -3
xcrun devicectl device install app --device 00008140-000158E80281401C \
...
xcrun devicectl device process launch --console --terminate-existing \
  --device 00008140-000158E80281401C com.openwhoop.OpenWhoop > /tmp/ow.log 2>&1 &
LP=$!
...
## 3. HOW REAL WHOOP TALKS TO THE STRAP (authoritative, from the decompiled APK `apk/`)

The decompiled WHOOP Android app is at **`apk/src/sources/`** (gitignored; obfuscated names). Authoritative
for "what WHOOP actually does." Key classes:
- `nz1/n.java` — the **sync session driver**. Offload = rate-limit ≥5 s → `GET_DATA_RANGE` (analytics
  only) → **plain `SEND_HISTORICAL_DATA(22)` with EMPTY payload** → ack loop. **No SET_CLOCK, no
  ENTER/EXIT high-freq, no R10/R11 toggle in the offload path.** On watchdog timeout (`n.java:2077`):
  `ABORT_HISTORICAL_TRANSMITS`, wait 3000 ms, retry. Command-response timeout = 5000 ms.
- `com/whoop/straphistorysync/sync/j.java` — offload reducer. Routes `HISTORY_START`→begin,
  `HISTORY_END`→ack chunk, `HISTORY_COMPLETE`→end (logs `totalSaved`); drops non-type-47 packets.
  Declares "Stuck" after **15** consecutive burst-validation failures.
- `com/whoop/service/rearchitect/c.java` — strap command layer. Connect lifecycle = **hello → set RTC →
  sync** (`:711` "on exchanging hello", `:716` "on setting rtc"). On `RtcLostEventPacket` it **re-syncs the
  clock** (`:982`). Drift check `>= 2 s` (`:262`). Strap-pushed events trigger syncs (`:1103-1145`,
  incl. `HighFreqSyncPromptEventPacket` → "Triggering 1 minute sync" = a normal offload, NOT enter-high-freq).
- Packet builders in `lh0/`: `s0.java` = **SET_CLOCK = 8 bytes `[seconds u32 LE][subseconds u32 LE]`**,
  subsec in 1/32768 s (`k82/a.java`); `e.java` = **ENTER_HIGH_FREQ_SYNC = 5 bytes
  `[0x02][interval u16 LE][duration u16 LE]`** (duration ≤7200, auto-exits) — built **only** in
  `com/whoop/data/repositories/SmartAlarmTriggerRepository.java` (Smart-Alarm ONLY); `f.java` = EXIT (empty);
  `s.java` = **SEND_HISTORICAL_DATA = EMPTY payload, fire-and-forget**; `u0.java` = SET_FF (65 bytes,
  see §4.5); `q.java` = HISTORICAL_DATA_RESULT ack (9 bytes, see §4.2).
- `so0/e.java` — the command-code enum. `vo0/a.java` — event numbers (RTC_LOST=13, SET_RTC=16, BOOT=15,
  FLASH_INIT_COMPLETE=28, BLE_SYSTEM_INITIALIZED=45).

**Bottom line on WHOOP's model:** persistent bonded connection; set the RTC on connect; offload with plain
...
### 4.1 Command codes (from `whoomp/scripts/packet.py` + `apk so0/e.java`)
...
5. **Why is GET_CLOCK(11) silent specifically?** This is the crux. Compare the exact GET_CLOCK frame our app
   sends vs `re/` vs the APK builder (`lh0/l.java`). Is the command code/payload right? Does GET_CLOCK respond
...
- Reference clones (gitignored): `whoomp/` (JS+Python reimpl; `whoomp/scripts/packet.py` = CommandNumber,
  `whoomp/whoomp.js`), `apk/src/sources/` (decompiled WHOOP app — see §3 for the classes).
