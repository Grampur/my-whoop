...
- Auth: `Authorization: Bearer <WHOOP_API_KEY>`
  (⚠️ this key is committed in the repo — **rotate it** before any public exposure.)
...
```bash
xcrun devicectl device copy from --device 00008140-000158E80281401C \
...
4. **The raw "flood" = R10/R11 realtime, controlled by `SEND_R10_R11_REALTIME(63)` payload
   ...
   ~2/s type-43 flood and persists across reconnect; the iOS connect handshake sends it. Found via APK
   `apk/src/sources/lh0/m0.java` + `n0.java`. The sigproc feature-flags (`sigproc_10_sec_dp`,
...
- Device: iPhone udid `00008140-000158E80281401C`, team `X4226N37ZF`, bundle `com.openwhoop.OpenWhoop`.
  Strap CB UUID `C1FBAF26-…` (in `re/device_config.py`). Build+install:
  `cd ios && xcodegen generate && xcodebuild -project OpenWhoop.xcodeproj -scheme OpenWhoop
  -destination 'id=00008140-000158E80281401C' -derivedDataPath build/dd -allowProvisioningUpdates
  DEVELOPMENT_TEAM=X4226N37ZF CODE_SIGN_STYLE=Automatic build` →
  `xcrun devicectl device install app --device 00008140-000158E80281401C build/dd/Build/Products/Debug-iphoneos/OpenWhoop.app`.
