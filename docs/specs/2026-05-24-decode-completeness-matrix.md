...
- **Verified?** names the SOURCE of confidence:
  ...
  - `from-app` — confirmed against the decompiled WHOOP Android app v5.452.0 enum/builders.
...
| 151 | GET_BATTERY_PACK_INFO | ✅ `0a03` | unsupported on this fw (new in v5.452 app) | on-device | 🟢 |
...
12. **`SET_DP_TYPE`(52) / `FORCE_DP_TYPE`(53) / `SET_RESEARCH_PACKET`(131)** — payloads unknown; these
    have NO command-builder and zero references in the v5.452 app, so the payload cannot be recovered
